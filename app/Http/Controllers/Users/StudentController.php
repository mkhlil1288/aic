...
class StudentController extends Controller
{
    ...
    {
		$student = \App\Models\Student::join('users','users.id','=','students.user_id')
	...
	public function class_routine(){
		...
		$data = array();
        $data['class'] = \App\Models\ClassModel::find($student->class_id);
        $data['section'] = \App\Models\Section::find($student->section_id);
        $data['routine'] = \App\Models\ClassRoutine::getRoutineView($student->class_id, $student->section_id);
	...
	public function stripe_payment($invoice_id){
		...
		$in= \App\Models\Invoice::find($invoice->id);
		$in->status = "Paid";
	...
	public function payment_history()
    {
		$studentpayments = \App\Models\StudentPayment::join('invoices','invoices.id','=','student_payments.invoice_id')
								...
								->get(); 								
        return view('backend.private.student.payment_history',compact('studentpayments','class'));
    }
	
	public function library_history()
    {
		$member = \App\Models\LibraryMember::join('users','users.id','=','library_members.user_id')->where('library_members.user_id',\Auth::user()->id)->first();
		$issues = array();
		if($member != NULL){
			$issues = \App\Models\BookIssue::select('*','book_issues.id AS id')
