...
class DashboardController extends Controller
{
	...
	private function Librarian_dashboard(){
		...
		$data['total_books'] = \App\Models\Book::selectRaw("SUM(quantity) as quantity")
		                                 ->first()->quantity;
										 
		$data['total_member'] = \App\Models\LibraryMember::selectRaw("COUNT(id) as total")
												  ->first()->total;
										 
		$data['issuing_book'] = \App\Models\BookIssue::selectRaw("COUNT(id) as total")
		                                      ->where("status",1)
		                                      ->first()->total;
