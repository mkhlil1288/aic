			...
			<tbody>
			  ...
			  <tr id="row_{{ $markdistribution->id }}">
					...
					@if ($markdistribution->is_exam == 'no')			
					  <form action="{{action('App\Http\Controllers\MarkDistributionController@destroy', $markdistribution['id'])}}" method="post">
						<a href="{{action('App\Http\Controllers\MarkDistributionController@edit', $markdistribution['id'])}}" data-title="{{ _lang('Update Mark Distribution') }}" class="btn btn-warning btn-sm">{{ _lang('Edit') }}</a>
						<a href="{{action('App\Http\Controllers\MarkDistributionController@show', $markdistribution['id'])}}" data-title="{{ _lang('View Mark Distribution') }}" class="btn btn-info btn-sm ajax-modal">{{ _lang('View') }}</a>
						{{ csrf_field() }}
