			 ...
			 @endif
				...
				<tbody>
				  ...
				  <tr id="row_{{ $grade->id }}">
					...
					<td>
					  <form action="{{action('App\Http\Controllers\GradeController@destroy', $grade['id'])}}" method="post">
						<a href="{{action('App\Http\Controllers\GradeController@edit', $grade['id'])}}" data-title="{{ _lang('Update Grade') }}" class="btn btn-warning btn-sm ajax-modal">{{ _lang('Edit') }}</a>
						<a href="{{action('App\Http\Controllers\GradeController@show', $grade['id'])}}" data-title="{{ _lang('View Grade') }}" class="btn btn-info btn-sm ajax-modal">{{ _lang('View') }}</a>
						{{ csrf_field() }}
