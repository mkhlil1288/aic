			...
			<tbody>
			  ...
			  <tr id="row_{{ $event->id }}">
				<td class='start_date'>{{ date('d/M/Y - H:i' ,strtotime($event->start_date)) }}</td>
					...
					<td>
					  <form action="{{action('App\Http\Controllers\EventController@destroy', $event['id'])}}" method="post">
						<a href="{{action('App\Http\Controllers\EventController@edit', $event['id'])}}" class="btn btn-warning btn-sm">{{ _lang('Edit') }}</a>
						<a href="{{action('App\Http\Controllers\EventController@show', $event['id'])}}" data-title="{{ _lang('View Event') }}" class="btn btn-info btn-sm ajax-modal">{{ _lang('View') }}</a>
						{{ csrf_field() }}
