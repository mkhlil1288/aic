		...
		<div class="panel panel-default no-export">
			...
			<tbody>
			  ...
			  <tr id="row_{{ $feetype->id }}">
				...
				<td>
				  <form action="{{action('App\Http\Controllers\FeeTypeController@destroy', $feetype['id'])}}" method="post">
					<a href="{{action('App\Http\Controllers\FeeTypeController@edit', $feetype['id'])}}" data-title="{{ _lang('Update Fee Type') }}" class="btn btn-warning btn-sm ajax-modal">{{ _lang('Edit') }}</a>
					<a href="{{action('App\Http\Controllers\FeeTypeController@show', $feetype['id'])}}" data-title="{{ _lang('View Fee Type') }}" class="btn btn-info btn-sm ajax-modal">{{ _lang('View') }}</a>
					{{ csrf_field() }}
