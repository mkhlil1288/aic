			...
			<tbody>
			  ...
			  @foreach($studentpayments as $studentpayment)
			    <tr id="row_{{ $studentpayment->id }}">
					...
					<td>
					  <form action="{{action('App\Http\Controllers\StudentPaymentController@destroy', $studentpayment['id'])}}" method="post">
						<a href="{{action('App\Http\Controllers\StudentPaymentController@edit', $studentpayment['id'])}}" data-title="{{ _lang('Update Student Payment') }}" class="btn btn-warning btn-sm ajax-modal">{{ _lang('Edit') }}</a>
						<a href="{{action('App\Http\Controllers\StudentPaymentController@show', $studentpayment['id'])}}" data-title="{{ _lang('View Student Payment') }}" class="btn btn-info btn-sm ajax-modal">{{ _lang('View') }}</a>
						{{ csrf_field() }}
