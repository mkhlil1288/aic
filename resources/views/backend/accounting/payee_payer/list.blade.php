			 ...
			 @endif
				...
				<tbody>
				  ...
				  <tr id="row_{{ $payeepayer->id }}">
					...
					<td>
					  <form action="{{action('App\Http\Controllers\PayeePayerController@destroy', $payeepayer['id'])}}" method="post">
						<a href="{{action('App\Http\Controllers\PayeePayerController@edit', $payeepayer['id'])}}" data-title="{{ _lang('Update Information') }}" class="btn btn-warning btn-sm ajax-modal">{{ _lang('Edit') }}</a>
						<a href="{{action('App\Http\Controllers\PayeePayerController@show', $payeepayer['id'])}}" data-title="{{ _lang('View Payee / Payer') }}" class="btn btn-info btn-sm ajax-modal">{{ _lang('View') }}</a>
						{{ csrf_field() }}
