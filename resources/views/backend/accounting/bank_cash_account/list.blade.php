		...
		<div class="panel panel-default no-export">
			...
			<tbody>
			  ...
			  <tr id="row_{{ $account->id }}">
				...
				<td>
				  <form action="{{action('App\Http\Controllers\AccountController@destroy', $account['id'])}}" method="post">
					<a href="{{action('App\Http\Controllers\AccountController@edit', $account['id'])}}" data-title="{{ _lang('Update Account') }}" class="btn btn-warning btn-sm ajax-modal">{{ _lang('Edit') }}</a>
					<a href="{{action('App\Http\Controllers\AccountController@show', $account['id'])}}" data-title="{{ _lang('View Account') }}" class="btn btn-info btn-sm ajax-modal">{{ _lang('View') }}</a>
					{{ csrf_field() }}
