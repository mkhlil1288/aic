				  ...
				  <tr id="row_{{ $role->id }}">
					...
					<td>
					  <form action="{{action('App\Http\Controllers\RoleController@destroy', $role['id'])}}" method="post">
						<a href="{{action('App\Http\Controllers\RoleController@edit', $role['id'])}}" data-title="{{ _lang('Update Permission Role') }}" class="btn btn-warning btn-sm ajax-modal">{{ _lang('Edit') }}</a>
						<a href="{{action('App\Http\Controllers\RoleController@show', $role['id'])}}" data-title="{{ _lang('View Permission Role') }}" class="btn btn-info btn-sm ajax-modal">{{ _lang('View') }}</a>
						{{ csrf_field() }}
