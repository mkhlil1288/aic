		...
		<div class="panel panel-default no-export">
			...
			<tbody>
			  ...
			  <tr id="row_{{ $picklist->id }}">
				...
				<td>
				  <form action="{{action('App\Http\Controllers\PicklistController@destroy', $picklist['id'])}}" method="post">
					<a href="{{action('App\Http\Controllers\PicklistController@edit', $picklist['id'])}}" data-title="{{ _lang('Update Picklist') }}" class="btn btn-warning btn-sm ajax-modal">{{ _lang('Edit') }}</a>
					{{ csrf_field() }}
