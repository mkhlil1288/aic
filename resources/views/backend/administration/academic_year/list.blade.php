			  ...
			  <table class="table table-bordered data-table">
			    ...
			    @foreach($academicyears as $academicyear)
				  <tr id="row_{{ $academicyear->id }}">
					...
					<td>
					  <form action="{{action('App\Http\Controllers\AcademicYearController@destroy', $academicyear['id'])}}" method="post">
						<a href="{{action('App\Http\Controllers\AcademicYearController@edit', $academicyear['id'])}}" data-title="{{ _lang('Update Academic Year') }}" class="btn btn-warning btn-sm ajax-modal">{{ _lang('Edit') }}</a>
						{{ csrf_field() }}
