				  ...
				  <tr id="row_{{ $invoice->id }}">
					...
					<td>
					  <div class="dropdown">
						  ...
						  <form action="{{action('App\Http\Controllers\InvoiceController@destroy', $invoice['id'])}}" method="post">			
							{{ csrf_field() }}
							<input name="_method" type="hidden" value="DELETE">
							<ul class="dropdown-menu">
							  <li><a href="{{ action('App\Http\Controllers\InvoiceController@edit', $invoice['id']) }}">{{ _lang('Edit') }}</a></li>
							  <li><a href="{{ action('App\Http\Controllers\InvoiceController@show', $invoice['id']) }}" data-title="{{ _lang('View Invoice') }}" data-fullscreen="true" class="ajax-modal">{{ _lang('View Invoice') }}</a></li>
